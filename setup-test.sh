...
#     sleep 2
#   done
#   fail "$label did not become reachable on port $port after 80s"
    sleep 10
}
...
sleep 5
#   ${PROXY_PORT}:8080 > "$PF_LOG" 2>&1 &
# PF_PID=$!
# sleep 1
# if ! kill -0 $PF_PID 2>/dev/null; then
#   fail "port-forward exited immediately. Log: $(cat $PF_LOG)"
# fi
# wait_for_port "$PROXY_PORT" "wake-proxy-${USER_ID}"
